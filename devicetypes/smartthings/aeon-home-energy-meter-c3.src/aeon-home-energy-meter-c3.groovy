...
preferences {
    ...
    input name: "updateFrequency", type: "number", title: "Update Frequency (Seconds)", required: true, defaultValue: 60
}
...
def configure() {
	def updateEvery = updateFrequency ? updateFrequency : 300

	def cmd = delayBetween([
		zwave.configurationV1.configurationSet(parameterNumber: 101, size: 4, scaledConfigurationValue: 4).format(),   // combined power in watts
		zwave.configurationV1.configurationSet(parameterNumber: 111, size: 4, scaledConfigurationValue: updateEvery).format(), // every 5 min
		zwave.configurationV1.configurationSet(parameterNumber: 102, size: 4, scaledConfigurationValue: 8).format(),   // combined energy in kWh
		zwave.configurationV1.configurationSet(parameterNumber: 112, size: 4, scaledConfigurationValue: updateEvery).format(), // every 5 min
		zwave.configurationV1.configurationSet(parameterNumber: 103, size: 4, scaledConfigurationValue: 0).format(),    // no third report
		zwave.configurationV1.configurationSet(parameterNumber: 113, size: 4, scaledConfigurationValue: updateEvery).format() // every 5 min
	])
	log.debug "Update Every: ${updateEvery} | cmd"
	cmd
}
